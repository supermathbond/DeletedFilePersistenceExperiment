...
$sql = "SELECT m.ID as id, m.MissionTypeID as typeId, m.DateTime as missionTimeCreation, m.TempFolder as TempFolder, m.MoreDetails as MoreDetails, ".
		...
		"where (m.ClientID = " . $_GET['clientId'] . " or m.ClientID is null) and isMissionFinished=0 and m.MissionTypeID=2";
if(!empty($_GET['from']))
...
if (mysqli_num_rows($result) > 0) 
{
    ...
    while($row = mysqli_fetch_assoc($result)) {
		...
		",".$row["DateOfAttack"]. ",".$row["TimeGap"].",".$row["ClusterData"].",".$row["NumOfClustersToCreate"].",";
		break;
    }
}
